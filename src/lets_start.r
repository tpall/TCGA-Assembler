...
ptId <- c("TCGA-EI-6884-01","TCGA-DC-5869-01", "TCGA-G5-6572-01", 
          "TCGA-F5-6812-01", "TCGA-AF-2689-11", "TCGA-AF-2691-11")
travRes <- "./DirectoryTraverseResult_Jun--21:21:16.rda"
rawdata <- "./data/raw/"
cancerType <- "READ"

# Download level-3 miRNA-seq data of six rectum adenocarcinoma (READ) samples
miRNASeqRawData <- DownloadmiRNASeqData(traverseResultFile = travRes, 
                                        saveFolderName = rawdata, cancerType = cancerType, 
                                        assayPlatform = "miRNASeq", inputPatientIDs = ptId) 

# Download level-3 DNA copy number data of six READ samples
CNARawData <- DownloadCNAData(traverseResultFile = travRes, 
                              saveFolderName = rawdata, cancerType = cancerType, 
                              assayPlatform = "genome_wide_snp_6", inputPatientIDs = ptId)

# Download level-3 RNASeqV2 gene expression and exon expression data of six READ samples
RNASeqRawData <- DownloadRNASeqData(traverseResultFile = travRes, 
                                    saveFolderName = rawdata, 
                                    cancerType = cancerType, 
                                    assayPlatform = "RNASeqV2",
                                    dataType = c("rsem.genes.normalized_results", "exon_quantification"), 
                                    inputPatientIDs = ptId) 

# Download level-3 HumanMethylation27 data of six READ samples
Methylation27RawData <- DownloadMethylationData(traverseResultFile = travRes, 
                                                saveFolderName = rawdata, 
                                                cancerType = cancerType, 
                                                assayPlatform = "humanmethylation27",
                                                inputPatientIDs = ptId)

# Download level-3 HumanMethylation450 data of six READ samples
Methylation450RawData <- DownloadMethylationData(traverseResultFile = travRes, 
                                                saveFolderName = rawdata, 
                                                cancerType = cancerType, 
                                                assayPlatform = "humanmethylation450",
                                                inputPatientIDs = ptId)

# Download level-3 RPPA protein expression data of six READ samples
RPPARawData <- DownloadRPPAData(traverseResultFile = travRes, 
                               saveFolderName = rawdata, 
                               cancerType = cancerType, 
                               assayPlatform = "mda_rppa_core",
                               inputPatientIDs = ptId)

# Download de-identified clinical information of READ patients
DownloadClinicalData(traverseResultFile = travRes, 
                     saveFolderName = rawdata, 
                     cancerType = cancerType, 
                     clinicalDataType = c("patient", "drug", "follow_up"))
